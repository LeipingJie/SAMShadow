...
class IoU(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray, file=None, name=None):
        pred, gt = _prepare_data(pred, gt)

        ious = self.cal_iou(pred=pred, gt=gt)
        self.ious.append(ious)
        if file != None:
            file.write(name+' '+str(ious.mean())+'\n')
...
class Fmeasure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray, file=None, name=None):
        ...
        if file != None:
            file.write(name+' '+str(changeable_fms.mean())+'\n')
...
class Ber(object):
    def __init__(self):
        """
        BER(balance error rate) for shadow detection.
        """
        self._TP = []
        self._TN = []
        self._Np = []
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray): 
        n_tn, n_tp, n_n, n_p = self.compute_ber(gt, pred)
        self._TP.append(n_tp)
        self._TN.append(n_tn)
        self._Np.append(n_p)
        self._Nn.append(n_n)
        self.total += gt.shape[0]*gt.shape[1]

    def get_results(self) -> dict:
        """
        Return the results about BER.

        :return: dict(ber=ber)
        """
        TP, TN, Np, Nn = sum(self._TP), sum(self._TN), sum(self._Np), sum(self._Nn)
        print('True Positive: ',TP)
        print('True Negative: ',  TN)
        print('Total Positive: ', Np)
        print('Total Negative: ', Nn)
        print('Total: ', self.total)

        accuracy = (TP+TN) * 1.0 / self.total                
        ber = (1 - 0.5*(TP/Np+TN/Nn))*100
              
        ber = (1 - 0.5*(TP/Np+TN/Nn))*100
        shadow = (1 - TP/Np)*100
        noshadow = (1 - TN/Nn)*100
        
        return dict(accuracy=accuracy, ber=ber, shadow=shadow, noshadow=noshadow)
    
    def compute_ber(self, gt, pred, interpolate=True):
        if interpolate:
            pred = (resize(pred/pred.max(), gt.shape)>0.1).astype(np.int)
        
        N_p = np.sum(pred)
        N_n = np.sum(np.logical_not(pred))

        TP = np.sum(np.logical_and(gt, pred))
        TN = np.sum(np.logical_and(np.logical_not(gt), np.logical_not(pred)))
        
        return TN, TP, N_n, N_p   